...
export function stripPrivateMagic(b64: string): string {
  return b64.startsWith(PRIVATE_MAGIC_B64) ? b64.slice(PRIVATE_MAGIC_B64.length) : b64; // backward-compat if old items lack the marker
}
...
export const QDeckId = {
  ...
  commentsPublic: (boardId: string, cardId: string) => QDeckCommentsId.publicV2(boardId, cardId),
  ...
  cardPrimaryImagePublic: (boardId: string, cardId: string) =>
    `qdeck__pubcimg__${boardId}__${cardId}`,
  cardPrimaryImagePrivate: (boardId: string, cardId: string) =>
...
export const QDeckCommentsId = {
  // Public comments (V2)
  publicV2: (boardId: string, cardId: string) => `qdeck_pub__cmv2__${boardId}__${cardId}`,
