...
export async function upvoteCard({
  ...
  await appendPaymentLine(issuerName, board, {
    ...
    amount,
    from,
    to: `${ownerAddr}+${revenueAddr}`,
    note:
      currency === 'QORT'
        ? `${percentSplit} based upon payment in ${currency} and (in future) board owner rating. QORT upvotes 20% goes to project owner. Pay in Q-Asset for Discount.`
...
export async function contributeBounty({
  ...
  amount: number;
  from: string;
  projectOwnerAddress: string; // hold in project wallet/treasury for now
}) {
  ...
  await appendPaymentLine(issuerName, board, {
    ...
    note: `${percentSplit}% based on ${currency} sent to Q-Assets Escrow, 
